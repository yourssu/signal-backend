...
Admin은 DEV와 PROD에 각각 실행하며 Docker 내부 네트워크의 같은 환경 Spring만 호출한다. `/t`, `/add`, `/delete`, `/report`, `/cancel`의 Slack Request URL은 PROD로, `/dev`의 Request URL은 DEV로 연결한다. PROD 전용 명령은 Slack 앱 콘솔에 슬래시 명령으로 직접 등록해야 하며, 등록하지 않으면 DEV의 `/dev` 서브커맨드는 동작해도 PROD 호출만 실패한다. 새 명령을 추가하면 등록 여부를 배포 전에 확인한다. Admin도 Spring, observer와 같은 3회 실패/1회 자동 재시작 정책을 적용한다.
