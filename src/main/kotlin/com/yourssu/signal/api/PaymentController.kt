...
class PaymentController(
    ...
    fun initiateKakaoPay(
        ...
        val response = paymentService.initiateKakaoPay(request.toCommand(uuid))
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(Response(result = response))
    }

    @Operation(
        summary = "토스페이먼츠 결제 초기화",
        description = "티켓 구매를 위한 토스페이먼츠 결제 프로세스를 초기화합니다. 결제 승인을 위한 URL을 반환합니다.",
        security = [SecurityRequirement(name = "bearerAuth")]
    )
    @PostMapping("/tosspayments/initiate")
    @RequireAuth
    fun initiateTossPayments(
        @Valid @RequestBody request: PaymentInitiationRequest,
        @Parameter(hidden = true) @UserUuid uuid: String
    ): ResponseEntity<Response<TossPaymentInitiationResponse>> {
        val response = paymentService.initiateTossPayments(request.toCommand(uuid))
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(Response(result = response))
    }
    ...
    fun approveKakaoPay(
        ...
        val paymentResponse = paymentService.approveKakaoPay(request.toCommand(uuid))
        return ResponseEntity.ok(Response(result = paymentResponse))
    }

    @Operation(
        summary = "토스페이먼츠 결제 승인",
        description = "사용자 승인 후 토스페이먼츠 결제 프로세스를 완료합니다. 결제 성공 시 티켓을 발급합니다.",
        security = [SecurityRequirement(name = "bearerAuth")]
    )
    @PostMapping("/tosspayments/approve")
    @RequireAuth
    fun approveTossPayments(
        @Valid @RequestBody request: PaymentApprovalRequest,
        @Parameter(hidden = true) @UserUuid uuid: String
    ): ResponseEntity<Response<PaymentCompletionResponse>> {
        val paymentResponse = paymentService.approveTossPayments(request.toCommand(uuid))
        return ResponseEntity.ok(Response(result = paymentResponse))
    }
}
